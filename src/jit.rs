...
impl LLJIT<'_> {
    ...
    pub fn add_module(&self, jdl: JDL, module: inkwell::module::Module) {
        unsafe {
            LLVMConsumeError(LLVMOrcLLJITAddLLVMIRModule(*self.0, jdl, LLVMOrcCreateNewThreadSafeModule(module.as_mut_ptr(), LLVMOrcCreateNewThreadSafeContext())));
        }
    }
    pub fn add_object(&self, jdl: JDL, obj: inkwell::memory_buffer::MemoryBuffer) {
        unsafe {
            LLVMConsumeError(LLVMOrcLLJITAddObjectFile(*self.0, jdl, obj.as_mut_ptr()));
        }
    }
