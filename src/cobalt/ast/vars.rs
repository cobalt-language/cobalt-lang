        ...
        if self.global || is_static {
            if is_extern.is_some() {
                ...
                match if let Some(v) = val.comp_val {
                    if ctx.is_const.get() {
                        ctx.with_vars(|v| v.insert(&self.name, Symbol::Variable(Value {export: true, ..val})))
                    }
                    else {
...
impl AST for ConstDefAST {
    ...
    fn codegen<'ctx>(&self, ctx: &CompCtx<'ctx>) -> (Value<'ctx>, Vec<Diagnostic>) {
        ...
        match ctx.with_vars(|v| v.insert(&self.name, Symbol::Variable(Value {export: true, ..val}))) {
            Ok(x) => (x.as_var().unwrap().clone(), errs),
            Err(RedefValue::NotAModule(x, _)) => {
                errs.push(Diagnostic::error(self.name.ids[x - 1].1.clone(), 321, Some(format!("{} is not a module", self.name.start(x)))));
                (Value::error(), errs)
...
impl AST for VarGetAST {
    ...
    fn codegen<'ctx>(&self, ctx: &CompCtx<'ctx>) -> (Value<'ctx>, Vec<Diagnostic>) {
        match ctx.with_vars(|v| v.lookup(&self.name)) {
            ...
            Err(UndefValue::NotAModule(idx)) => (Value::error(), vec![Diagnostic::error(self.name.ids[idx].1.clone(), 321, Some(format!("{} is not a module", self.name.start(idx))))]),
            Err(UndefValue::DoesNotExist(idx)) => (Value::error(), vec![Diagnostic::error(self.name.ids[idx].1.clone(), 320, Some(format!("{} does not exist", self.name.start(idx))))])
        }
    }
    fn to_code(&self) -> String {
        format!("{}", self.name)
