...
pub struct Interner<T: Eq + Hash + Send + Sync + 'static, S = RandomState>(Lazy<InternerInner<T>>, MaybeLazy<S>);
...
impl<T: Eq + Hash + Send + Sync + 'static, S> Interner<T, S> {
    pub const fn with_hasher(state: S) -> Self {
        Self(Lazy::new(|| InternerInner::new(Herd::new(), |_| ThreadLocal::new(), |_| RwLock::default())), MaybeLazy::Eager(state))
    }
}
impl<T: Eq + Hash + Send + Sync + 'static, S: Default> Interner<T, S> {
    pub const fn new() -> Self {
        Self(Lazy::new(|| InternerInner::new(Herd::new(), |_| ThreadLocal::new(), |_| RwLock::default())), MaybeLazy::Lazy(Lazy::new(S::default)))
    }
}
impl<T: Eq + Hash + Send + Sync + 'static, S: BuildHasher> Interner<T, S> {
    pub fn intern<'a, Q: Eq + Hash + Into<T>>(
        &'a self,
        key: Q,
    ) -> &'a T
    where
        T: Borrow<Q>
    {
        ...
        self.0.with_set(|set| {
            ...
            if let Some(r) = lock.find(hash, |m| <T as Borrow<Q>>::borrow(m) == &key) {
                ...
                unsafe {
                    crate::new_lifetime(&*r)
                }
            } else {
                std::mem::drop(lock);
                let ret = self.0.with_member(|m| unsafe {
                    crate::new_lifetime(m.get_or(|| crate::new_lifetime(self.0.borrow_owner()).get()).alloc(key.into()))
                });
                set.write().insert_unique(hash, ret, |v| builder.hash_one(&v));
                &*ret
            }
        })
    }
    pub fn intern_ref<'a, 'b, Q: Eq + Hash + ToOwned + ?Sized>(
        &'a self,
        key: &'b Q,
    ) -> &'a T
    where
        ...
        Q::Owned: Into<T>
    {
        ...
        self.0.with_set(|set| {
            ...
            if let Some(r) = lock.find(hash, |m| <T as Borrow<Q>>::borrow(m) == key) {
                ...
                std::mem::drop(lock);
                let ret = self.0.with_member(|m| unsafe {
                    crate::new_lifetime(m.get_or(|| crate::new_lifetime(self.0.borrow_owner()).get()).alloc(key.to_owned().into()))
                });
                set.write().insert_unique(hash, ret, |v| builder.hash_one(&v));
                &*ret
            }
...
impl<T: Eq + Hash + Send + Sync + 'static, S> Drop for Interner<T, S> {
    fn drop(&mut self) {
        self.0.with_set_mut(|set| {
            set.get_mut().iter_mut().for_each(|p| unsafe {
                std::ptr::drop_in_place::<T>(*p as *const T as *mut T)
            })
        })
    }
}
